...
async function createReport(req, res) {
  try {
    ...
    await logAndEmailUser(user_id, reporter?.email, "Report Submitted",
      `Your report against ${report_user.first_name} ${report_user.last_name} has been submitted and is being reviewed.`
    );
    await notifySuperAdmins(`New report filed against ${report_user.first_name} ${report_user.last_name}`, 'warning');
